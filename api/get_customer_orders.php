...
if ($search !== '') {
    $whereSql .= ' AND (o.order_number LIKE ? OR v.business_name LIKE ?)';
    $like = '%' . $search . '%';
    $params[] = $like;
    $params[] = $like;
    $types .= 'ss';
...
$countStmt = $conn->prepare('SELECT COUNT(*) AS total FROM orders o LEFT JOIN vendors v ON v.vendor_id = o.vendor_id' . $whereSql);
if (!$countStmt) {
    apiError('DB_QUERY_ERROR', 'Failed to prepare customer order count query.', 500);
}
...
$query = 'SELECT o.order_id, o.order_number, o.vendor_id, v.business_name AS vendor_name, v.business_category AS vendor_category, v.avg_rating AS vendor_rating, o.order_status, o.payment_status, o.order_date, o.expected_delivery_date, o.actual_delivery_date, o.shipping_address, o.billing_address, o.subtotal, o.discount_amount, o.tax_amount, o.shipping_cost, o.commission_amount, o.total_amount, o.currency, o.shipping_method, o.tracking_number, o.carrier_name, o.notes, o.customer_notes, o.return_status, o.created_at, o.updated_at FROM orders o LEFT JOIN vendors v ON v.vendor_id = o.vendor_id' . $whereSql . ' ORDER BY o.created_at DESC LIMIT ? OFFSET ?';
$stmt = $conn->prepare($query);
if (!$stmt) {
    apiError('DB_QUERY_ERROR', 'Failed to prepare customer orders query.', 500);
}
$finalParams = $params;
$finalTypes = $types . 'ii';
...
], 'Customer orders retrieved successfully.', 'CUSTOMER_ORDERS_RETRIEVED');
